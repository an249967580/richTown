...
{
    public class TableView : MonoBehaviour, ITableView, IEndDragHandler
    {
        #region Property
        ...
        private void RemoveCellsIfNeededWithRanges(UnityEngine.SocialPlatforms.Range previousRange, UnityEngine.SocialPlatforms.Range currentRange)
        {
        ...
        private void CreateCellsIfNeededWithRanges(UnityEngine.SocialPlatforms.Range previousRange, UnityEngine.SocialPlatforms.Range currentRange)
        {
        ...
        {
            UnityEngine.SocialPlatforms.Range startRange = new UnityEngine.SocialPlatforms.Range(0, visibleCells.Range.from);
            float startSize = cellSizes.SumWithRange(startRange);

            UnityEngine.SocialPlatforms.Range endRange = new UnityEngine.SocialPlatforms.Range(visibleCells.Range.from, visibleCells.Range.Last() + 1);
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (layoutOrientation == TableViewLayoutOrientation.Vertical)
            {
                //下拉刷新
                if (scroller.verticalNormalizedPosition > 1.001f)
                {
                    if (tableViewDelegate != null)
                    {
                        tableViewDelegate.TableViewDidScrollToStart(this);
                    }
                }
                //上拉加载
                if (scroller.verticalNormalizedPosition < 0.001f)
                {
                    if (tableViewDelegate != null)
                    {
                        tableViewDelegate.TableViewDidScrollToEnd(this);
                    }
                }
            }
            else
            {
                //右拉刷新
                if (scroller.horizontalNormalizedPosition > 1.001f)
                {
                    if (tableViewDelegate != null)
                    {
                        tableViewDelegate.TableViewDidScrollToStart(this);
                ...
                //左拉加载
                if (scroller.horizontalNormalizedPosition < 0.001f)
                {
                    if (tableViewDelegate != null)
                    {
                        tableViewDelegate.TableViewDidScrollToEnd(this);
